"""Script to load questions from a PDF using regexes from config.json
and print all questions for a given chapter to the console.

Database support has been removed for this script.
"""
...
def main() -> None:
    ...
    # 1. add support for taking a chapter name as input in the command line
    chapter_input = input("Enter the chapter name: ").strip()
    # 1. Handle case where empty string is provided as input
    ...
    matched_questions: list[tuple[str, str]] = []

    # Chapter-wise parsing, but only for the requested chapter name
    for chap_match in chapter_pattern.finditer(full_text):
        ...
        # Only process chapters matching the user-provided name (case-insensitive)
        if chapter_name.lower() != chapter_input.lower():
            continue
        ...
        for match in question_pattern.finditer(chapter_text):
            ...
            matched_questions.append((question_text, options))

    # 2. Handle case where there are no questions corresponding to the provided chapter name
    if not matched_questions:
        print(f"No questions found for chapter '{chapter_input}'.")
        return

    # 3. Print all questions on the console
    print(f"\nQuestions for chapter '{chapter_input}':\n")
    for idx, (q_text, q_options) in enumerate(matched_questions, start=1):
